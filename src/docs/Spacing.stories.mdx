...
Values are defined by a specific set of design tokens, i.e. for different projects, these values might differ. For Phoenix components, we use Axiory design tokens by default. These values can be found in [Design tokens](?path=/docs/foundation-design-tokens--page). Please see the tokens starting with `ref`:
...
Margin and padding props in Phoenix components are unified and are used by many components where applicable. They use use Spacing values as defined above and include these props:
